...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			this.log(`[auth] Error initiating Z Code Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate Z Code Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from Z Code Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with Z Code Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid Z Code Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with Z Code Cloud")
			}

			this.log("[auth] Successfully authenticated with Z Code Cloud")
		} catch (error) {
			this.log(`[auth] Error handling Z Code Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle Z Code Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from Z Code Cloud")
			}

			this.log("[auth] Logged out from Z Code Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from Z Code Cloud: ${error}`)
			throw new Error(`Failed to log out from Z Code Cloud: ${error}`)
		}
	}
