...
> **Scope clarification (重要修正)：**
>
> 本文档描述的是一个**对齐 Aider 原版 Repo Map 行为的目标态（target-state）架构与算法设计**，用于指导 Rust 语言的完整实现。
>
> 它**不等同于**当前代码库中已经存在的 Repo Map / Repo Graph / 可视化实现的“完成状态”。
> 在当前系统中，许多关键阶段（如 definition–reference 解析、加权依赖图构建、Personalized PageRank、token 预算裁剪）仍处于缺失或占位阶段。
>
> 因此，本文应被视为：**架构蓝图与完成度标尺**，而非现状描述。
...
> **重要修正：Tag 的语义权重问题**
>
> 在 Aider 的真实实现中，Tag 并不是一个“语法层面的平权数据结构”。
> Tag 在进入图构建与排序阶段之前，已经隐式或显式地携带了多种**语义权重信号**。

#### **3.1.1 Tag Weight Semantics（Aider 与简化模型的关键差异）**

在 Aider 中，Tag 的影响力并非一致，主要受到以下因素共同作用：

* **可见性 / 导出权重（Visibility / Export）**  
  公有或导出的符号（如 Rust 中的 `pub fn`、`pub struct`）通常具有更高权重，因为它们更可能被跨文件引用。

* **跨文件引用频率（Cross-File Reference Frequency）**  
  一个定义如果被多个文件引用，其对应的文件间边（edges）会被多次累加权重，而不是简单的二值存在关系。

* **聊天上下文命中（Chat Context Boost）**  
  来自当前 chat / edit 上下文文件的 Tag 会获得额外偏置，该信号随后会被 Personalized PageRank 的 personalization vector 放大。

* **结构性角色（Structural Role Heuristics）**  
  类型定义、核心数据结构、配置入口点等，往往比叶子层 helper 函数更具语义中心性。Aider 使用轻量级启发式规则对此进行偏置。

**本文前文中给出的 Tag 结构是“语法正确但语义简化”的模型，适合作为流水线说明，但不足以完整复刻 Aider 的排序行为。**

在完整实现中，Tag 至少应被视为：

```text
Tag = Syntax × Visibility × Usage × Context
```
