...
export interface ExtensionStateContextType extends ExtensionState {
	...
	terminalZdotdir?: string
	setTerminalZdotdir: (value: string) => void
	...
	setRepoMapEnabled: (value: boolean) => void
}
...
export const ExtensionStateContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	const [state, setState] = useState<ExtensionState>({
		...
		terminalZdotdir: "/tmp", // Default ZDOTDIR handling setting
	...
	const [repoMapEnabled, setRepoMapEnabled] = useState(true)
	...
	const [terminalZdotdir, setTerminalZdotdir] = useState("/tmp")
	const [fuzzyMatchThreshold, setFuzzyMatchThreshold] = useState(1.0)
	const [alwaysAllowModeSwitch, setAlwaysAllowModeSwitch] = useState(false)
	const [alwaysAllowSubtasks, setAlwaysAllowSubtasks] = useState(false)
	const [alwaysAllowReadOnly, setAlwaysAllowReadOnly] = useState(false)
	const [alwaysAllowReadOnlyOutsideWorkspace, setAlwaysAllowReadOnlyOutsideWorkspace] = useState(false)
	const [alwaysAllowWrite, setAlwaysAllowWrite] = useState(false)
	const [alwaysAllowWriteOutsideWorkspace, setAlwaysAllowWriteOutsideWorkspace] = useState(false)
	const [alwaysAllowExecute, setAlwaysAllowExecute] = useState(false)
	const [alwaysAllowBrowser, setAlwaysAllowBrowser] = useState(false)
	const [alwaysAllowMcp, setAlwaysAllowMcp] = useState(false)
	const [browserToolEnabled, setBrowserToolEnabled] = useState(true)
	const [showRooIgnoredFiles, setShowRooIgnoredFiles] = useState(true)
	const [enableSubfolderRules, setEnableSubfolderRules] = useState(false)
	const [telemetrySetting, setTelemetrySetting] = useState<TelemetrySetting>("unset")
	const [screenshotQuality, setScreenshotQuality] = useState(75)
	const [terminalOutputLineLimit, setTerminalOutputLineLimit] = useState(500)
	const [terminalOutputCharacterLimit, setTerminalOutputCharacterLimit] = useState(50000)
	const [terminalShellIntegrationTimeout, setTerminalShellIntegrationTimeout] = useState(4000)
	const [terminalShellIntegrationDisabled, setTerminalShellIntegrationDisabled] = useState(false)
	const [terminalCompressProgressBar, setTerminalCompressProgressBar] = useState(true)
	const [includeDiagnosticMessages, setIncludeDiagnosticMessages] = useState(true)
	const [maxDiagnosticMessages, setMaxDiagnosticMessages] = useState(50)
	const [maxOpenTabsContext, setMaxOpenTabsContext] = useState(20)
	const [maxWorkspaceFiles, setMaxWorkspaceFiles] = useState(200)
	const [maxReadFileLine, setMaxReadFileLine] = useState(-1)
	const [maxImageFileSize, setMaxImageFileSize] = useState(5)
	const [maxTotalImageSize, setMaxTotalImageSize] = useState(20)
	const [writeDelayMs, setWriteDelayMs] = useState(1000)
	const [browserViewportSize, setBrowserViewportSize] = useState("900x600")
	const [mode, setMode] = useState<Mode>(defaultModeSlug)
	const [customModes, setCustomModes] = useState<ModeConfig[]>([])
	const [customModePrompts, setCustomModePrompts] = useState<CustomModePrompts>(defaultPrompts)
	const [customSupportPrompts, setCustomSupportPrompts] = useState<CustomSupportPrompts>({})
	const [enhancementApiConfigId, setEnhancementApiConfigId] = useState("")
	const [condensingApiConfigId, setCondensingApiConfigId] = useState("")
	const [customCondensingPrompt, setCustomCondensingPrompt] = useState("")
	const [hasOpenedModeSelector, setHasOpenedModeSelector] = useState(false)
	const [autoApprovalEnabled, setAutoApprovalEnabled] = useState(false)
	const [historyPreviewCollapsed, setHistoryPreviewCollapsed] = useState(false)
	const [reasoningBlockCollapsed, setReasoningBlockCollapsed] = useState(true)
	...
	const [codebaseIndexConfig, setCodebaseIndexConfig] = useState<any>({
		codebaseIndexEnabled: true,
		codebaseIndexQdrantUrl: "http://localhost:6333",
		codebaseIndexEmbedderProvider: "openai",
		codebaseIndexEmbedderBaseUrl: "",
	...
	const [codebaseIndexModels, setCodebaseIndexModels] = useState<any>({ ollama: {}, openai: {} })
	const [enableCheckpoints, setEnableCheckpoints] = useState(true)
	const [checkpointTimeout, setCheckpointTimeout] = useState(DEFAULT_CHECKPOINT_TIMEOUT_SECONDS)
	const [remoteBrowserHost, setRemoteBrowserHost] = useState("")
	const [modeApiConfigs, setModeApiConfigs] = useState<Record<string, string>>({})
	const [remoteBrowserEnabled, setRemoteBrowserEnabled] = useState(false)
	const [currentApiConfigName, setCurrentApiConfigName] = useState("default")
	...
	const contextValue: ExtensionStateContextType = {
		...
		repoMapEnabled,
		setRepoMapEnabled: (value: boolean) => {
			setRepoMapEnabled(value)
			vscode.postMessage({ type: "updateSettings", updatedSettings: { repoMapEnabled: value } })
		},
	}
