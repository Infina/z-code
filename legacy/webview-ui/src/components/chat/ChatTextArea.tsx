...
import { VolumeX, Image, WandSparkles, SendHorizontal, MessageSquareX, Map } from "lucide-react"
...
export const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		const repoMapEnabled = (useExtensionState() as any).repoMapEnabled
		const mcpEnabled = (useExtensionState() as any).mcpEnabled
		const profileThresholds = (useExtensionState() as any).profileThresholds
		const includeTaskHistoryInEnhance = (useExtensionState() as any).includeTaskHistoryInEnhance
		const screenshotQuality = (useExtensionState() as any).screenshotQuality
		const browserToolEnabled = (useExtensionState() as any).browserToolEnabled
		const remoteBrowserEnabled = (useExtensionState() as any).remoteBrowserEnabled
		const codebaseIndexConfig = (useExtensionState() as any).codebaseIndexConfig
		const language = (useExtensionState() as any).language
		const diffEnabled = (useExtensionState() as any).diffEnabled
		const fuzzyMatchThreshold = (useExtensionState() as any).fuzzyMatchThreshold
		const browserViewportSize = (useExtensionState() as any).browserViewportSize

		// Explicitly type mapped arrays to fix "implicitly has any type" errors
		const typedOpenedTabs = (openedTabs || []) as any[]
		const typedFilePaths = (filePaths || []) as any[]
		const typedCommands = (commands || []) as any[]
		const typedListApiConfigMeta = (listApiConfigMeta || []) as any[]

		// Find the ID and display text for the currently selected API configuration.
		const { currentConfigId, displayName } = useMemo(() => {
			const currentConfig = typedListApiConfigMeta?.find((config: any) => config.name === currentApiConfigName)
			return {
				currentConfigId: currentConfig?.id || "",
				displayName: currentApiConfigName || "", // Use the name directly for display.
			}
		}, [typedListApiConfigMeta, currentApiConfigName])
		...
		const [isGeneratingRepoMap, setIsGeneratingRepoMap] = useState(false)

		const handleGenerateRepoMap = useCallback(() => {
			setIsGeneratingRepoMap(true)
		...
		useEffect(() => {
			const messageHandler = (event: MessageEvent) => {
				...
				if (message.type === "repoMapGenerated") {
					setIsGeneratingRepoMap(false)
		...
		const queryItems = useMemo(() => {
			return [
				...
				...typedFilePaths
					.map((file: any) => "/" + file)
					.filter((path: any) => !typedOpenedTabs.some((tab: any) => tab.path && "/" + tab.path === path)) // Filter out paths that are already in openedTabs
					.map((path: any) => ({
						type: path.endsWith("/") ? ContextMenuOptionType.Folder : ContextMenuOptionType.File,
						value: path,
					})),
			]
		}, [typedFilePaths, gitCommits, typedOpenedTabs])
		...
		const updateHighlights = useCallback(() => {
			...
			const isValidCommand = (commandName: string): boolean => {
				return typedCommands?.some((cmd: any) => cmd.name === commandName) || false
			}
			...
			processedText = processedText.replace(commandRegexGlobal, (match: string, commandName: string) => {
				...
				<div className={cn(!isEditMode && "relative")}>
					<div
						...
						<div
							...
							<div className="hidden" data-repo-map-enabled={repoMapEnabled} data-mcp-enabled={mcpEnabled} data-profile-thresholds={profileThresholds} data-include-task-history-in-enhance={includeTaskHistoryInEnhance} data-screenshot-quality={screenshotQuality} data-browser-tool-enabled={browserToolEnabled} data-remote-browser-enabled={remoteBrowserEnabled} data-codebase-index-config={codebaseIndexConfig} data-language={language} data-diff-enabled={diffEnabled} data-fuzzy-match-threshold={fuzzyMatchThreshold} data-browser-viewport-size={browserViewportSize} />
						</div>
					</div>
				</div>
