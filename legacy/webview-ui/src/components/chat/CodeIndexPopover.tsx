...
export const CodeIndexPopover: React.FC<CodeIndexPopoverProps> = ({
	...
	useEffect(() => {
		const handleMessage = (event: MessageEvent) => {
			if (event.data.type === "repoMapGenerated") {
					...
					<div className="p-4">
						...
						<div className="space-y-2">
							<div className="flex items-center justify-between">
								<h4 className="text-sm font-medium m-0">{t("settings:codeIndex.statusTitle")}</h4>
								<div className="flex items-center gap-2">
									{repoMapData?.files && (
										<span className="text-xs text-vscode-descriptionForeground">
											{repoMapData.files.length} files, {repoMapData.edges?.length || 0} connections
										</span>
									)}
									<StandardTooltip content={t("chat:generateRepoMap") || "Generate Repository Map"}>
										<Button
											variant="ghost"
											size="sm"
											disabled={isGeneratingRepoMap}
											onClick={handleGenerateRepoMap}
											className="h-7 w-7 p-0 hover:bg-vscode-toolbar-hoverBackground">
											<Map
												className={cn(
													"w-4 h-4",
													isGeneratingRepoMap && "animate-pulse text-vscode-charts-orange",
												)}
											/>
										</Button>
									</StandardTooltip>
									<StandardTooltip content={t("chat:openRepoMap") || "Open Repository Map"}>
										<Button
											variant="ghost"
											size="sm"
											onClick={() => window.postMessage({ type: "openCodebaseStatusDialog" }, "*")}
											className="h-7 w-7 p-0 hover:bg-vscode-toolbar-hoverBackground ml-1">
											<span className="codicon codicon-screen-full text-sm" />
										</Button>
									</StandardTooltip>
								</div>
							</div>
