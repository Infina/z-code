...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
	...
	const { repoMapData: mapData, setRepoMapData } = useRepoMap()
	// 强制可见性：只要 context 里有数据就展示
	const isMapVisible = useMemo(() => !!(mapData && mapData.files && mapData.files.length > 0), [mapData])
	...
	const [playNotification] = useSound(`${audioBaseUri}/notification.wav`, { volume, soundEnabled: !!soundEnabled })
	const [playCelebration] = useSound(`${audioBaseUri}/celebration.wav`, { volume, soundEnabled: !!soundEnabled })
	const [playProgressLoop] = useSound(`${audioBaseUri}/progress_loop.wav`, { volume, soundEnabled: !!soundEnabled })
	...
	return (
		<div
			...
			{/* 全息图现在放在这里，不受 task 是否存在的限制 */}
			<div className={cn("flex flex-col px-3 relative", !isMapVisible && "hidden")}>
				<div className="absolute top-2 right-5 z-20">
					<Button
						...
						className="h-6 w-6 p-0 bg-vscode-sideBar-background border border-vscode-widget-border hover:bg-vscode-toolbar-hoverBackground">
						<span className="codicon codicon-close text-xs" />
					</Button>
				</div>
				<HolographicMap data={mapData} visible={isMapVisible} />
			</div>
