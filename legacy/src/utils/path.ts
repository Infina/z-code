...
export const getWorkspacePath = (defaultCwdPath = "") => {
	...
	const cwdPath = workspaceFolders?.map((folder) => folder.uri.fsPath).at(0) || defaultCwdPath
	const currentFileUri = vscode.window.activeTextEditor?.document.uri

	let detectedRoot = cwdPath
	if (currentFileUri) {
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(currentFileUri)
		detectedRoot = workspaceFolder?.uri.fsPath || cwdPath
	}

	// Smart Root Anchoring: 向上寻找最近的 package.json 或 .git
	if (detectedRoot) {
