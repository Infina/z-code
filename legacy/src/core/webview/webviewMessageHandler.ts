...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "openKeyboardShortcuts": {
			// Open VSCode keyboard shortcuts settings and optionally filter to show the Z Code commands
		...
		case "openRepoMap":
			await vscode.commands.executeCommand("roo.openRepoMap")
			break
		case "generateRepoMap": {
			try {
				const repoMapService = RepoMapService.getInstance(provider.context)
				// 深度路径回退机制
				const workspaceFolder = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath
				const rawCwd = provider.cwd || workspaceFolder || getWorkspacePath() || ""
				// 强制使用真实的绝对路径，避免因为空格或符号链接导致的偏差
				const cwd = fs_sync.realpathSync(path.resolve(path.normalize(rawCwd)))
				const workspaceName = path.basename(cwd);
				
				console.log(`[webviewMessageHandler] Attempting to generate map for CWD (Resolved): "${cwd}"`)
				
				// 强制切换进程工作目录，确保 Rust 引擎的相对路径计算起点正确
				try {
					process.chdir(cwd);
					console.log(`[webviewMessageHandler] Changed process.cwd() to: "${process.cwd()}"`);
				} catch (err) {
					console.error(`[webviewMessageHandler] Failed to change directory: ${err}`);
				}
				console.log(`[webviewMessageHandler] Node.js dirname: "${__dirname}"`)
				console.log(`[webviewMessageHandler] process.cwd(): "${process.cwd()}"`)

				if (!cwd || cwd.trim() === "") {
					console.error("[webviewMessageHandler] No workspace path found during generateRepoMap")
					vscode.window.showErrorMessage("No workspace path available for repo map generation.")
					await provider.postMessageToWebview({ type: "repoMapGenerated" })
					break
				}
				...
				console.log(`[webviewMessageHandler] Calling repoMapService.generateJsonMap with binaryPath: ${repoMapService["binaryPath"]} for CWD: ${cwd}`)
				const repoMap = await repoMapService.generateJsonMap(cwd, openFiles)
				console.log(`[webviewMessageHandler] RAW repoMap files:`, repoMap.files.map(f => f.path))
				console.log(`[webviewMessageHandler] repoMap generated:`, JSON.stringify(repoMap).substring(0, 500))
				console.log(`[webviewMessageHandler] repoMap generated with ${repoMap.files?.length || 0} files`)
				
				// 转换为相对路径，以适配前端显示逻辑 (如果需要)
				if (repoMap.files) {
					repoMap.files = repoMap.files.map((f: any) => {
						const absPath = path.resolve(f.path);
						let relPath = path.relative(cwd, absPath);
						
						// 极致对齐逻辑：
						// 1. 如果路径以 workspaceName 开头，说明 Node.js 包含冗余父目录
						// 2. 否则，保持 relative 结果
						const parts = relPath.split(path.sep);
						const wsIdx = parts.indexOf(workspaceName);
						if (wsIdx !== -1) {
							relPath = parts.slice(wsIdx + 1).join(path.sep);
						}

						console.log(`[webviewMessageHandler] Final Path mapping: "${f.path}" -> "${relPath}"`);
						return {
							...f,
							path: relPath
						};
				...
				if (repoMap["edges"]) {
					...
					repoMap["edges"] = edges.map((e: any) => {
						const absSource = path.resolve(e.source);
						const absTarget = path.resolve(e.target);
						let relSource = path.relative(cwd, absSource);
						let relTarget = path.relative(cwd, absTarget);

						const sParts = relSource.split(path.sep);
						const sWsIdx = sParts.indexOf(workspaceName);
						if (sWsIdx !== -1) relSource = sParts.slice(sWsIdx + 1).join(path.sep);
						
						const tParts = relTarget.split(path.sep);
						const tWsIdx = tParts.indexOf(workspaceName);
						if (tWsIdx !== -1) relTarget = tParts.slice(tWsIdx + 1).join(path.sep);
						
						console.log(`[webviewMessageHandler] Final Edge mapping: "${e.source}" -> "${relSource}" | "${e.target}" -> "${relTarget}"`);
				...
				await provider.postMessageToWebview({
					type: "repoMapUpdate",
					text: JSON.stringify(repoMap),
				})
				await provider.postMessageToWebview({
					type: "repoMapGenerated",
				})
			} catch (error) {
				console.error(`[webviewMessageHandler] Error generating repo map:`, error)
				provider.log(`Error generating repo map: ${error}`)
				vscode.window.showErrorMessage(`Failed to generate repo map: ${error}`)
				await provider.postMessageToWebview({
					type: "repoMapGenerated",
				})
			}
