...
export class OpenAICompatibleEmbedder implements IEmbedder {
	...
	private async makeDirectEmbeddingRequest(
		...
		// Gemini's OpenAI compatibility endpoint expects Google-style API key auth.
		// Adding this header is harmless for other providers and makes Gemini work
		// when using the direct-fetch code path.
		try {
			...
			// If URL parsing fails, skip provider-specific headers.
		}
