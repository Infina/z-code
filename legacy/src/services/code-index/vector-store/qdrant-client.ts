...
export class QdrantVectorStore implements IVectorStore {
	...
	private parseQdrantUrl(url: string | undefined): string {
		...
		try {
			// Attempt to parse as complete URL - normalize known user mistakes.
			const parsedUrl = new URL(trimmedUrl)

			// Common mistake: people paste the collections endpoint instead of the base URL.
			// If we keep "/collections" as the pathname, the QdrantClient will treat it as a prefix
			// and generate incorrect URLs (e.g. /collections/collections/...)
			if (parsedUrl.pathname === "/collections" || parsedUrl.pathname === "/collections/") {
				parsedUrl.pathname = "/"
