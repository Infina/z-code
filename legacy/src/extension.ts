...
import { RepoMapWebviewProvider } from "./core/webview/RepoMapWebviewProvider"
...
export async function activate(context: vscode.ExtensionContext) {
	...
	vscode.window.showInformationMessage("Welcome to Z Code: The Last AI-native Editor!")
	...
	if (process.env.NODE_ENV !== "development") {
		...
		console.log("[Z Code] Development mode: Skipping PostHog registration")
	}
	...
	// Initialize the provider *before* the Z Code Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)
    const repoMapProvider = new RepoMapWebviewProvider(context.extensionUri, outputChannel)

	// Initialize Z Code Cloud service.
	const postStateListener = () => {
        ClineProvider.getVisibleInstance()?.postStateToWebview()
        repoMapProvider.postStateToWebview(provider)
    }
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("roo.openRepoMap", async () => {
            repoMapProvider.show()
            // Push initial state
            const state = await provider.getStateToPostToWebview()
            repoMapProvider.postMessage({ type: "state", state })
        })
    )
	...
	// Phase 2: Launch Rust Holographic Map Engine
	// In development mode, the extension runs from the 'src' directory (see launch.json),
	// but the binary is built into the project root's 'bin' directory.
	const roostPath =
		context.extensionMode === vscode.ExtensionMode.Development
			? path.join(context.extensionPath, "..", "bin", "roost_bin")
			: path.join(context.extensionPath, "bin", "roost_bin")

	outputChannel.appendLine(`[Z Code] Attempting to launch Rust Engine at ${roostPath}`)

	// Store the path globally so system prompt can use it
	context.globalState.update("roost_bin_path", roostPath)
