...
/**
 * Fetches available models from the Z Code Cloud provider
 *
 * @param baseUrl The base URL of the Z Code Cloud provider
 * @param apiKey The API key (session token) for the Z Code Cloud provider
...
export async function getRooModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		try {
			...
			if (!parsed.success) {
				console.error("Error fetching Z Code Cloud models: Unexpected response format", data)
				console.error("Validation errors:", parsed.error.format())
				throw new Error("Failed to fetch Z Code Cloud models: Unexpected response format.")
			}
		...
		console.error("[getRooModels] Error fetching Z Code Cloud models:", {
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch Z Code Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch Z Code Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch Z Code Cloud models: No response from server. Check Z Code Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch Z Code Cloud models: ${error.message || "An unknown error occurred."}`)
	}
}
